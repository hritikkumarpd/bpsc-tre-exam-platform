  ...
  it('should fail cleanly if eligible question pool has fewer than 150 unique questions', () => {
    ...
    try {
      if (eligibleCount < requiredCount) {
        throw new Error(
          `Mock generation failed. Insufficient unique questions available in pool. Required: 150, Available: ${eligibleCount}.`
        );
      }
    } catch (err) {
      errorThrown = true;
      expect((err as Error).message).toContain('Insufficient unique questions');
    }
