...
async function importAllTREPYQs() {
  ...
  for (const config of pyqPapersConfig) {
    const textPath = path.join(scratchDir, config.fileName);
    ...
    const parsedQuestions = parseCleanEnglishQuestions(rawText);
    ...
    const existingPaper = await PYQPaperModel.findOne({ slug: config.slug });
    if (existingPaper) {
      await PYQQuestionModel.deleteMany({ pyqPaperId: existingPaper._id });
      await PYQPaperModel.deleteOne({ _id: existingPaper._id });
