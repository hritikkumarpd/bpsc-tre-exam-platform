...
export const isEnglishText = (text: string): boolean => {
  ...
  if (hindiGarbleRegex.test(text)) return false;
...
export const generateSolutionForQuestion = (
  questionText: string,
  options: { key: string; text: string }[],
  correctAnswer: string
): { explanation: string; subject: string; topic: string } => {
...
export const parseCleanEnglishQuestions = (rawText: string): CleanEnglishQuestion[] => {
  ...
  const parsedQuestions: CleanEnglishQuestion[] = [];
  let currentNum = 1;
  ...
  for (const block of questionBlocks) {
    ...
    if (!isEnglishText(trimmed)) continue;
    ...
    if (!isEnglishText(questionText)) continue;

    const options: { key: 'A' | 'B' | 'C' | 'D' | 'E'; text: string }[] = [];
    const validKeys: ('A' | 'B' | 'C' | 'D' | 'E')[] = ['A', 'B', 'C', 'D', 'E'];

    for (const m of optionMatches) {
      const key = m[1].toUpperCase() as any;
      const text = m[2].trim();

      // Only include options with clean English text
      if (validKeys.includes(key) && isEnglishText(text) && !options.some((o) => o.key === key)) {
    ...
    let correctAnswer: 'A' | 'B' | 'C' | 'D' | 'E' = 'A';
    const ansMatch = trimmed.match(/(?:Answer|Ans|Correct Option)\s*:\s*([A-E])/i);
    if (ansMatch && validKeys.includes(ansMatch[1].toUpperCase() as any)) {
    ...
    const { explanation, subject, topic } = generateSolutionForQuestion(questionText, options, correctAnswer);
